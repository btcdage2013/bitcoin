...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
    ...
    // -----------------------------------------------------------------
    // 阶段一: 瞬发挖矿 (Instant Mining Phase)
    // 在指定高度之前，锁定难度为网络允许的最低值，以完成空投。
    // 注意: 我们要挖的是新区块，其高度为 pindexLast->nHeight + 1。
    // -----------------------------------------------------------------
    const int nInstantMineEndHeight = 55555; // 定义瞬发挖矿阶段的结束高度 (区块 0 到 60000)
                                             // 在测试时，您可以临时将其改为 1001
    ...
    // 在瞬发挖矿阶段结束后，平滑切换到 Dark Gravity Wave 算法。
    // -----------------------------------------------------------------
